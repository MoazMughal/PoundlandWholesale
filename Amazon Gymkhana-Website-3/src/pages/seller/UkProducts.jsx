...
const SellerUkProducts = () => {
  ...
  const loadUkProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch(getApiUrl('excel/products'));
      ...
      alert('❌ Could not load UK products from server');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleListProducts = async () => {
    ...
    const productsToList = Array.from(selectedProducts).map(index => {
      ...
      return {
        name: product.title || product.Title || 'Untitled Product',
        ...
        reviews: parseInt(product.reviews || product.Reviews || 0),
    ...
    try {
      ...
      if (response.ok) {
        const message = seller?.verificationStatus === 'approved' 
          ? `✅ Successfully listed ${data.imported} UK products to Amazon's Choice!`
          : `✅ Successfully submitted ${data.imported} UK products for approval!`;
        alert(message);
    ...
    return (
      <div className="container-fluid mt-3">
        <div className="text-center py-5">
          ...
          <p className="mt-3">Loading UK products...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="d-flex justify-content-between align-items-center mb-2" style={{padding: '4px 0'}}>
        <div>
          <h6 className="mb-1" style={{fontSize: '0.9rem', fontWeight: '600'}}>
            <i className="fas fa-file-excel text-primary me-2"></i>
            UK Products ({products.length}) | Selected: {selectedProducts.size}
          </h6>
            ...
            <table className="table table-hover" style={{tableLayout: 'fixed', width: '100%'}}>
              <thead>
                <tr>
                  ...
                  <th style={{fontSize: '0.75rem', padding: '8px'}}>Your Price (GBP)</th>
                  ...
                  <tr key={index} style={{fontSize: '0.8rem'}}>
                    ...
                    <td style={{padding: '8px'}}>
                      <div style={{maxWidth: '300px', fontSize: '0.8rem'}}>
                        <strong>{product.title || product.Title || product.name || product.Name || 'Untitled'}</strong>
                        {(product.brand || product.Brand) && <div className="text-muted" style={{fontSize: '0.7rem'}}>{product.brand || product.Brand}</div>}
                      </div>
                    </td>
                    <td style={{padding: '8px'}}>
                      <div className="input-group input-group-sm" style={{width: '120px'}}>
                        ...
                        <input
                          ...
                          style={{fontSize: '0.75rem'}}
                          value={editedData[index]?.price ?? (product.price || product.Price || 0)}
                    ...
                    <td style={{padding: '8px'}}>
                      <input
                        ...
                        value={editedData[index]?.stock ?? (product.stock || product.Stock || 100)}
...
export default SellerUkProducts;