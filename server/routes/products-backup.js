...
async function getProductsWithFallback(query = {}, options = {}) {
  try {
    ...
    if (query.isAmazonsChoice) {
      fallbackFiltered = fallbackFiltered.filter(p => p.isAmazonsChoice === true);
    }
    if (query.category && query.category !== 'all') {
      fallbackFiltered = fallbackFiltered.filter(p => p.category === query.category);
    }
    
    return {
      products: fallbackFiltered.slice(0, options.limit || 50),
      total: fallbackFiltered.length,
      totalPages: Math.ceil(fallbackFiltered.length / (options.limit || 50)),
...
router.get('/public', async (req, res) => {
  try {
    ...
    // Optimized query with lean() for better performance and timeout handling
    let products;
    try {
      ...
      console.error('❌ Database query timeout or error:', queryError.message);
      
      // Return empty result set on timeout
      if (queryError.name === 'MongoNetworkTimeoutError' || queryError.name === 'MongoTimeoutError') {
        return res.json({
          products: [],
          totalPages: 0,
    ...
    // Only count if needed for pagination with timeout handling
    let count;
    try {
      count = parseInt(limit) < 200 ? await Product.countDocuments(query).maxTimeMS(5000) : products.length;
    } catch (countError) {
