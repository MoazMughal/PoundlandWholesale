...
class AuthManager {
  ...
  // Initialize authentication on app load - PERSISTENT MODE
  ...
  async _performInitialization() {
    try {
      console.log('🔄 Initializing authentication - PERSISTENT MODE...')
      
      // STEP 1: Check current page path
      const currentPath = window.location.pathname
      
      // STEP 2: Check if we have any auth data at all
      const activeUserType = localStorage.getItem('activeUserType')
      ...
      // STEP 3: Get the token for the active user type
      ...
      // STEP 4: Check if token is still valid (not expired)
      if (!this.isTokenValid(token)) {
      ...
      // STEP 5: Verify token with server (but be more forgiving)
      try {
