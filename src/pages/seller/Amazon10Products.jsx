...
const SellerAmazon10Products = () => {
  ...
  // Currency options for Amazon 10
  const currencies = [
  ...
  const loadAmazon10Products = async () => {
    try {
      ...
      alert('❌ Could not load Amazon 10 products from server');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleListProducts = async () => {
    ...
    const productsToList = Array.from(selectedProducts).map(index => {
      ...
      const productData = {
        ...
        image: getAmazon10ImagePath(product.asin),
        images: [getAmazon10ImagePath(product.asin)],
        currency: 'USD',
    ...
    try {
      ...
      if (response.ok) {
        const message = seller?.verificationStatus === 'approved' 
          ? `✅ Successfully listed ${data.imported} Amazon 10 products to Amazon's Choice!`
          : `✅ Successfully submitted ${data.imported} Amazon 10 products for approval!`;
        alert(message);
        setSelectedProducts(new Set());
        
        // Show errors if any
        if (data.errors && data.errors.length > 0) {
          console.error('Listing errors:', data.errors);
          alert(`⚠️ Some products had errors: ${data.errors.map(e => e.error).join(', ')}`);
        }
      } else {
        console.error('Bulk list error:', data);
      ...
      <div className="d-flex justify-content-between align-items-center mb-2" style={{padding: '4px 0'}}>
        <div>
          <h6 className="mb-1" style={{fontSize: '0.9rem', fontWeight: '600'}}>
            ...
            Amazon 10 Products ({products.length}) | Selected: {selectedProducts.size}
          </h6>
            ...
            <table className="table table-hover" style={{tableLayout: 'fixed', width: '100%'}}>
              <thead>
                <tr>
                  ...
                  <th style={{fontSize: '0.75rem', padding: '8px'}}>Your Price ({currency})</th>
...
export default SellerAmazon10Products;